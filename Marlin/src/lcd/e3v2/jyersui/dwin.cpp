...
void CrealityDWINClass::Draw_Status_Area(bool icons/*=false*/) {
  ...
  int16_t zoX = STATUS_X3, zoY = STATUS_Y2;		// Z-Offset
    ...
    sprintf_P(infoline, PSTR("pc:%i; pr:%i; pa=%i; wu:%i"), process, printing, paused, wait_for_user);
  ...
  switch (menu) {
    ...
    case ManualLevel:
      ...
      switch (item) {
        ...
        case MLEVEL_RL:   // = TL
          if (draw)
            Draw_Menu_Item(row, ICON_AxisTL, F("Rear Left"));
          else {
            Popup_Handler(MoveWait);
            if (use_probe) {
              #if HAS_BED_PROBE
        ...
        case MLEVEL_MR:
          if (draw)
            Draw_Menu_Item(row, ICON_AxisC, F("Mid Right"));
          else {
            Popup_Handler(MoveWait);
            if (use_probe) {
              #if HAS_BED_PROBE
        ...
        case MLEVEL_RR:   // = TR
          if (draw)
            Draw_Menu_Item(row, ICON_AxisTR, F("Rear Right"));
          else {
            Popup_Handler(MoveWait);
            if (use_probe) {
              #if HAS_BED_PROBE
      ...
      switch (item) {
        case INFO_BACK:
          if (draw) {
            ...
            Draw_Menu_Item(INFO_SIZE, ICON_PrintSize, F(MACHINE_SIZE), F(HW_FEATURE), false, true);
            Draw_Menu_Item(INFO_VERSION, ICON_Version, F(SHORT_BUILD_VERSION), F(__DATE__ " " __TIME__), false, true);
            Draw_Menu_Item(INFO_CONTACT, ICON_Contact, F(CORP_WEBSITE), F(GIT_BRANCH), false, true);
          }
          else {
            if (menu == Info)
